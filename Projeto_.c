...
int VerificarIgualdade(char TEXTO[MAX_CHAR]) { // tentando verificar nomes repetidos
    int Retorno=0;
    TEXTO = upper(TEXTO);

    for(int b=0; b < MAX_CHAR; b++) {
        if (strcmp(TEXTO, upperAtividade[b]) == 0) {
        Retorno = 1;
        break;
        }
    }
...
int main(){
    ...
    while (fgets(linha, sizeof(linha), arquivo)) != NULL) {
        ...
        atividade[i][] = linha;
        i++;
        upperAtividade[k][] = linha;
        k++;
    } // eu pensei em ir lendo cada linha do arquivo e ir atualizando os vetores para poderem ser comparados. Basicamente a modificação foi feita nos i e j dos for de leitura, pq (se isso der certo) meio que tem que atualizar as posiçoes com oq ja tem no arquivo
    ...
    while(flag==1){//loop do menu
        ...
        switch(opcao){
            case 1:
                ...
                do{
                    ...
                    do{
                        ...
                        retorno=VerificarEspacos(atividade[i]);
                        retorno =VerificaIgualdade(upperAtividade[i]);

                    }while(retorno);

                    atividade[i][strcspn(atividade[i], "\n")] = 0;//remove o \n q o fgets salva no final

                    for(; k<sizeof(atividade[i]); k++){ //aq tinha um int k = 0;
            ...
            case 2:
                ...
                printf("\n===============================");
                printf("\nA FAZER:");
                for(; j < contador; j++)  // em todos esses tinha um int j = 0
                    if (status[j] == 1) {
                        printf("\n - %s", atividade[j]);
                    }
                }
    ...
    arquivo=fopen("atividades.txt", "w");//lembrar de mudar parametro
    ...
    for(int p=0; p<contador; p++){
        fprintf(arquivo,"%d\n%s\n%s\n", status[p], atividade[p], upperAtividade[p]);//salva o status e a atividade no arquivo
    }
