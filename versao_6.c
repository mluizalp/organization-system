...
void escreve_dados(int status[], char atividade[][MAX_CHAR], char upperAtividade[][MAX_CHAR], int contador) {
    ...
    for (int p = 0; p < contador; p++) {//salva as linhas das matrizes q tem dados salvos
...
// Listagem limpa sem exibir o índice numérico
    ...
    if (contador > 0) {
        ...
        do {
            ...
            if (numero < 0 || numero >= contador) {
                printf("\nNumero de atividade invalido! Tente novamente.\n");
            }
        ...
        do {
            ...
            if (status[numero] < 1 || status[numero] > 3) {
                printf("\nStatus invalido! Escolha de 1 a 3.\n");
            }
...
// Busca adaptada para procurar por String/Nome da atividade
void buscar_atividade(int status[], char atividade[][MAX_CHAR], char upperAtividade[][MAX_CHAR], int contador) {
    ...
    if (contador <= 0) {
        printf("\nNenhuma atividade cadastrada para buscar.\n");
        return;
    }

    char buscar_ativ[MAX_CHAR];
    int validador = 0, pos = -1;
    int continuar = 1;
    ...
    do {
        ...
        // Deixa a string de busca em maiúsculo
        for (int j = 0; buscar_ativ[j] != '\0'; j++) {
            buscar_ativ[j] = toupper(buscar_ativ[j]);
        }

        // Percorre a lista comparando com a matriz em maiúsculo
        for (int j = 0; j < contador; j++) {
    ...
    if (status[pos] == 1) printf("A fazer\n");
    else if (status[pos] == 2) printf("Em andamento\n");
    else if (status[pos] == 3) printf("Concluida\n");
}
